          ...
          <div className="space-y-4">
            <div className="bg-green-50 border border-green-200 rounded-md p-4">
              ...
              <div className="bg-white border border-green-300 rounded p-2">
                <code className="text-sm text-gray-900 break-all">{createdApiKey}</code>
              </div>
            </div>
            <button
              onClick={handleClose}
...
function ProjectCard({ project, onDelete, onRegenerateKey }: ProjectCardProps) {
  ...
  const statsQuery = trpc.projects.stats.useQuery({
  ...
  const regenerateKey = trpc.projects.regenerateApiKey.useMutation({
    onSuccess: () => {
      onRegenerateKey();
    },
  });
    ...
    <div className="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow">
      ...
      {statsQuery.isLoading ? (
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          ...
          <div className="bg-gray-50 rounded p-3">
            <p className="text-xs text-gray-600 mb-1">Event Types</p>
            <p className="text-2xl font-bold text-gray-900">{stats.eventBreakdown.length}</p>
          </div>
        </div>
      ) : null}
      ...
      <div className="mb-4">
        ...
        {showApiKey && (
          <div className="mt-2 bg-gray-50 rounded p-2 border border-gray-200">
            <code className="text-xs text-gray-900 break-all">{project.apiKey}</code>
          </div>
        )}
      </div>
        ...
        <button
          onClick={() => {
            if (confirm('Are you sure you want to regenerate the API key? The old key will stop working.')) {
              regenerateKey.mutate({ projectId: project.projectId });
            }
          }}
      ...
      {showDeleteConfirm && (
        <div className="mt-4 bg-red-50 border border-red-200 rounded p-4">
          <p className="text-sm text-red-800 mb-3">
            Are you sure? This will delete the project and all its events permanently.
          </p>
          <div className="flex space-x-2">
            <button
            ...
            <button
              onClick={() => deleteProject.mutate({ projectId: project.projectId })}
    ...
    <div>
      ...
      {projectsQuery.error && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          <p className="text-red-800">Error loading projects: {projectsQuery.error.message}</p>
        </div>
      )}
